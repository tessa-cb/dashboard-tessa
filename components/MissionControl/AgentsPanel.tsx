...
export default function AgentsPanel({ agents }: { agents: Agent[] }) {
  ...
  const [viewingNotifications, setViewingNotifications] = useState<string | null>(null)
  ...
  const toggleNotifications = async (agent: Agent) => {
      ...
      try {
          const res = await fetch(`/api/notifications?agentId=${agent.id}&unread=true`)
          const data = await res.json()
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      ...
      <div className="space-y-3">
        {agents.map(agent => (
          <div key={agent.id} className="border rounded hover:bg-gray-50 transition-colors">
            <div className="flex items-center justify-between p-2">
                <div className="flex items-center gap-3 cursor-pointer" onClick={() => toggleNotifications(agent)}>
            ...
            {editingAgent === agent.id && (
                <div className="p-2 border-t bg-gray-50">
                    ...
                    <div className="flex justify-end gap-2">
                        <button onClick={() => setEditingAgent(null)} className="text-xs text-gray-500">Cancel</button>
                        <button onClick={() => saveKey(agent.id)} className="text-xs bg-blue-600 text-white px-2 py-1 rounded">Save</button>
                    </div>
                </div>
            )}
            ...
            {viewingNotifications === agent.id && (
                <div className="p-2 border-t bg-yellow-50">
                    <h4 className="text-xs font-bold mb-2 uppercase text-gray-500">Notifications</h4>
                    {notifications.length === 0 ? (
                        <div className="text-xs text-gray-400">No unread notifications</div>
                    ) : (
                        <div className="space-y-2">
                            {notifications.map(n => (
                                <div key={n.id} className="flex justify-between items-start text-xs bg-white p-2 rounded border">
                                    <span>{n.message}</span>
                                    <button onClick={() => markRead(n.id)} className="text-blue-600 hover:text-blue-800 ml-2">✓</button>
                                </div>
                            ))}
                        </div>
                    )}

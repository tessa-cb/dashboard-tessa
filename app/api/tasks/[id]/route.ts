...
export async function PATCH(
  ...
  try {
    ...
    if (status && status !== oldTask.status) {
      await logActivity('task_status', `Task "${task.title}" moved to ${status}`, { taskId: id, oldStatus: oldTask.status, newStatus: status })
    }
    
    if (agentId && agentId !== oldTask.agentId) {
       const agent = task.agent
       await logActivity('task_assigned', `Task "${task.title}" assigned to ${agent?.name || 'Unknown'}`, { taskId: id, agentId })
       ...
       if (status === 'in_progress') {
         ...
         await logActivity('agent_status', `${agent?.name} is now working`, { agentId })
       }
    }
